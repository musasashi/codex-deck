...
import { modelRequest, isExternalModel } from '../core/providers';
...
export async function checkResponsesModel(model: string, purpose: ProviderCheckPurpose, request: Request,
  signal: AbortSignal, progress: (check: ProviderCheck) => void = () => {},
  options: { label?: string; confirmation?: string; requireUsage?: boolean } = {}): Promise<ProviderCheck> {
  const label = options.label ?? 'Responses API', confirmation = options.confirmation ?? 'Responses tool OK';
  let stage = '応答・タスク名の要約';
  const update = () => progress({ model, purpose, status: 'checking', message: `${stage}を確認中…` });
  try {
    if (!isExternalModel(model)) throw new Error('APIのモデルIDを確認してください。');
    modelRequest(model);
    ...
    const call = async (body: JsonObject): Promise<JsonObject> => {
      ...
      if (!response.ok) {
        ...
        throw new Error(`${label}がリクエストを拒否しました（HTTP ${response.status}）。${detail}`);
      }
    ...
    if (purpose === 'task') {
      ...
      const second = await call({ tools, tool_choice: 'auto', input: [...input, ...array(first.output),
        { type: 'function_call_output', call_id: calls[0]!.call_id, output: confirmation }] });
      if (array(second.output).some(item => object(item).type === 'function_call')) throw new Error('ツールが再度呼び出され、最終回答を確認できませんでした。');
      if (!textOutput(second).includes(confirmation)) throw new Error('返却したツール結果を含む回答を確認できませんでした。');
    }
    stage = '利用トークン数'; update();
    const hasUsage = samples.every(usage => Number.isSafeInteger(usage.input_tokens) && Number(usage.input_tokens) > 0
      && Number.isSafeInteger(usage.output_tokens) && Number(usage.output_tokens) > 0);
    if (options.requireUsage && !hasUsage) throw new Error('利用トークン数が未取得または0のため、タスクの利用額を計算できません。');
    return { model, purpose, status: 'passed', message: (purpose === 'task' ? '利用可：応答・ツール往復を確認しました。' : '利用可：要約を確認しました。')
      + (hasUsage ? '利用トークン数を確認しました。' : '利用トークン数は未取得のため、費用は未計測です。') };
  } catch (error) {
    const message = signal.aborted ? '確認を中止しました。' : error instanceof Error && (error.name === 'TimeoutError' || error.name === 'AbortError')
      ? '確認がタイムアウトしました。再度お試しください。' : error instanceof Error ? error.message : `${label}への接続に失敗しました。`;
